...
function setup() {
  ...
  pisoinvisible = createSprite(50, height/2+25, 50, 5);
  pisoinvisible.visible = false;
  Suelo = createSprite(600, height/2+15);
  Suelo.addImage("Suelo", Suelo1);
  Suelo.velocityX = -5;

  //Suelo.scale=1.35;
  Trex = createSprite(50, height/2);
  ...
  //Trex al chocar
  TrexChoque=createSprite(50, height/2);
 ...
 GameOver.addImage("Fin",GameO);
  ...
  GameOver.visible=false;
   ...
   Reset=createSprite(width/2,height/3);
...
function Nubes() {
  if (frameCount % 55 === 0) {
    Nube = createSprite(width, Math.round(random(40, 140)));
...
function Cactus() {
  ...
  if (frameCount % 65 === 0) {
    Obstaculo = createSprite(width, Math.round(random(height/2, height/2-30)));
